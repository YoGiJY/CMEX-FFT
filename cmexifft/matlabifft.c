...
void ComputeData(Bcomplex *x1, Bcomplex *x2, Bcomplex *x3, Bcomplex *x4, Bcomplex r1,
	Bcomplex r2, Bcomplex r3, Bcomplex r4)
{

	long long brw1pr, biw1pi,
		 crw2pr, ciw2pi,
		 drw3pr, diw3pi,
		 brw1pi, biw1pr,
		 crw2pi, ciw2pr,
...
void fftport(double *data_in,double *data_out)
{
    ...
    for (int i = 0; i<SIZE; ++i)
    {
        Idata[i].real = (long long)data_in[2+i];
        Idata[i].image = (long long)data_in[2+i+4096];
    }
    LevelFftAddr();
    RomDataFromTxt();
    RomAddrGenerate();
